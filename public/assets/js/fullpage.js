...
(function(a) {
    a.fn.fullpage = function(b) {
        ...
        function da(c) {
            ...
            if (!P(c.target) && (c = a(".fp-section.active"), !t&&!p))
                if (d = Q(d), w = d.y, A = d.x, c.find(".fp-slides").length && Math.abs(B - A) > Math.abs(x - w))
                    Math.abs(B - A) >
                    a(window).width() / 100 * b.touchSensitivity && (B > A ? a.fn.fullpage.moveSlideRight() : a.fn.fullpage.moveSlideLeft());
                else if (b.autoScrolling && (d = c.find(".fp-slides").length ? c.find(".fp-slide.active").find(".fp-scrollable") : c.find(".fp-scrollable"), Math.abs(x - w) > a(window).height() / 100 * b.touchSensitivity))
                    if (x > w)
                        if (0 < d.length)
                            if (C("bottom", d))
        ...
        function C(c, a) {
            if ("top" === c)
                return !a.scrollTop();
            if ("bottom" === c)
                return a.scrollTop() + 1 + a.innerHeight() >= a[0].scrollHeight
        }
        function G(c) {
        ...
        function z(a) {
            ...
            if (e.length)
                ...
                f = a.get(0).scrollHeight, b.verticalCentered && (f = a.find(".fp-tableCell").get(0).scrollHeight);
            d = l - parseInt(d.css("padding-bottom")) - parseInt(d.css("padding-top"));
            f > d ? e.length ? e.css("height", d + "px").parent().css("height", d + "px") : (b.verticalCentered ? a.find(".fp-tableCell").wrapInner('<div class="fp-scrollable" />') : a.wrapInner('<div class="fp-scrollable" />'), a.find(".fp-scrollable").slimScroll({
                allowPageScroll: !0,
                height: d + "px",
                size: "10px",
                alwaysVisible: !0
        ...
        function $(a) {
            ...
            if (b.paddingTop || b.paddingBottom)
                d = a, d.hasClass("fp-section") || (d = a.closest(".fp-section")), a = parseInt(d.css("padding-top")) + parseInt(d.css("padding-bottom")),
                    d = l - a;
            return d
        }
        function U(a, b) {
        ...
        function ha() {
            var a = document.createElement("p"), b, e = {
            ...
            for (var f in e)
                void 0 !== a.style[f] && (a.style[f] = "translate3d(1px,1px,1px)", b = window.getComputedStyle(a).getPropertyValue(e[f]));
